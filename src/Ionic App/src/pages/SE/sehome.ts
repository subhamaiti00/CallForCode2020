...
import { vieworderSE } from "../SE/vieworderSE";
@Component({
  selector: 'page-sehome',
  templateUrl: 'sehome.html'
})
...
export class sehome {
  ...
  this.storage.get('uid').then((val) => {
    this.uid=val;
    this.http.get(this.apiurl+'/S1/service.ashx?method=sehome&uid='+this.uid).subscribe(data1 => {
        loader.dismiss();
        //alert(data1);
      if(data1==null || data1=="")
      {      
  ...
  orderdetails(ID,UName,Date,OrderValue,address,city,pin,Mobile)
  {
    this.nav.push(vieworderSE, {ID: ID,UName:UName,Date:Date,OrderValue:OrderValue,address:address,city:city,pin:pin,Mobile:Mobile});
  }
}
